...
def center_map_gen(center_map, x, y, label, sigma, g):
    ...
    # upper left. LHA: change from np.around -> removed
    ul = int((x - 3 * sigma - 1)), int((y - 3 * sigma - 1))
    # bottom right. LHA: change from np.around -> removed
...
def merge_gt_pred_segmap(softmax_seg_map, gt_seg_map):
    ...
    gt_seg_map = gt_seg_map.clone()
    gt_seg_map[gt_seg_map == 255] = 0
    for b in range(softmax_seg_map.shape[0]):
        for _cls in range(softmax_seg_map.shape[1]):
            # not giving priority to predicted seg for GT background
            _gt_seg_map[b, _cls] = ((gt_seg_map[b] == _cls) * (_cls != 0)) * 3
            # treating GT background the same as other GT classes
            _gt_seg_map[b, _cls] = (gt_seg_map[b] == _cls) * 3
            _gt_seg_map[b, _cls] += softmax_seg_map[b, _cls]
    return _gt_seg_map
    return _gt_seg_map.max(dim=1)[1]

def box_from_seg(seg_map, gt_seg_map, args):
    """
    Box generation from semantic segmentation
    Arguments:
    ...
    B, _, H, W = seg_map.shape

    prob_map = F.softmax(seg_map, dim=1)        # [B, C+1, H, W]
    seg_map = merge_gt_pred_segmap(prob_map, gt_seg_map).cpu().max(dim=1)[1]
    ...
    for b in range(B):
        ...
        _valid_cls = torch.unique(_seg_map[(_seg_map != 0) & (_seg_map != 255)])-1
        ...
        for _cls in _valid_cls:
            ...
            all_small = np.all([stats[k, cv2.CC_STAT_AREA] < MINIMUM_MASK_SIZE
                                for k in range(1, n_contours)])

            # all connected components of this class _cls
            for k in range(1, n_contours): # starting from 1 to skip BG
                size = stats[k, cv2.CC_STAT_AREA]

                if size < MINIMUM_MASK_SIZE and not all_small:
                    continue

                mask = torch.from_numpy(contours == k)
                ymin, ymax = y_coord[mask].min(), y_coord[mask].max()
                xmin, xmax = x_coord[mask].min(), x_coord[mask].max()

                if  (xmax - xmin) < EPSILON or (ymax - ymin) < EPSILON:
                    continue

                _targets.append([j/s for (j, s) in zip(
                    [xmin, ymin, xmax, ymax], [W, H, W, H])] + [_cls+1])
                    # [_cls, conf])

        targets.append(torch.Tensor(_targets).to(device))

    return {
        'targets': targets,
    }

def refine_box_generation(seg_map, center_map, offset_map, gt_seg_map, args):
    """
    Refined-box generation with image-level labels.
    Arguments:
        seg_map: A Tensor of shape [B, C+1, H, W]. output semantic segmentation map.
    ...
    seg_map = gt_seg_map
    # LHA: 1. create a reference GT = GT + Pred segmentation
    seg_map = merge_gt_pred_segmap(prob_map, gt_seg_map)
    seg_scr, seg_map = seg_map.cpu().max(dim=1)
    ...
    for b in range(B):
        ...
        for _cls in _valid_cls:
            ...
            #adding center points of miss detected segments
            n_centers, centers, _, _ = cv2.connectedComponentsWithStats(
                (__center_map.numpy() > 0).astype(np.uint8), connectivity=8
            )
            ...
            for k in range(1, n_contours): # starting from 1 to skip BG
                ...
                contour_mask = torch.from_numpy(contour_mask)

                for c in range(1, n_centers):
                    # if contour_mask[centers == c].any():
                    __cnts = __center_map[(centers == c) & (contours == k)]
                    if len(__cnts) > 0 and __cnts.max() > thr:
                        break
                else: # no center for this blob
                    seg_ = _seg_scr * contour_mask
                    cy, cx = torch.nonzero(seg_ == seg_.max())[0] # torch nonzero gives (i,j)
                    _center_map = center_map_gen(
                        _center_map,
                        cx, cy, _cls,
                        args.sigma, g,
                    )
                ...
                # for c in range(1, n_centers):
                #     if contour_mask[centers == c].any():
                #         scores.append(__center_map[centers == c].max())
                # if len(scores) == 0:
                #     _center_map = center_map_gen(
                #         _center_map,
                #         cx, cy, _cls,
                #         args.sigma, g,
                #     )
                # elif len(scores) > MAXIMUM_NUM_INST:
                #     topk = np.array(scores).argsort()[-MAXIMUM_NUM_INST:][::-1]
                #     for c in 

                # remove center points belong to NO gt segmentation
                __c_center_map = (__center_map * contour_mask).to(device)

                __ins_seg = get_instance_segmentation(contour_mask[None, ...].to(device),
                                                      __c_center_map[None, None, ...],
                                                      _offset_map[None, ...],
                                                      threshold=thr, #args.refine_thresh,
                ...
                for i in range(1, n_ins+1):
                    ...
                    if mask.sum() > 0:
                        ...
                        """  using seg_mask as ins_mask, its center is contour's center"""
...
def refine_label_generation(seg_map, center_map, offset_map, label, gt_seg_map, args):
    ...
    # LHA: 1. create a reference GT = GT + Pred segmentation
    seg_map = merge_gt_pred_segmap(prob_map, gt_seg_map)
    seg_scr, seg_map = seg_map.max(dim=1)
        ...
        for _cls in _valid_cls:
            ...
            for k in range(1, n_contours): # starting from 1 to skip BG
                ...
                for c in range(1, n_centers):
                    ...
                    seg_ = seg_scr.squeeze() * contour_mask
                    seg_[seg_ < seg_.max()] = 0
                    cy, cx = torch.nonzero(seg_)[0] # torch nonzero gives (i,j)
                    _center_map = center_map_gen(
                        _center_map,
                        cx, cy, _cls,
                        args.sigma, g,
...
def refine_label_generation_with_point(seg_map, gt_point_cls, offset_map, label,
                                       gt_seg_map, args):
    """
...
def get_instance_segmentation(fg, ctr_hmp, offsets, threshold=0.1, nms_kernel=3,
                              top_k=None, ignore=True, beta=5):
    """
    # This implementation is from https://github.com/bowenc0221/panoptic-deeplab.
