...
if __name__ == '__main__':
    ...
    epoch_size = {t: len(train_sets[t]) // args.batch_size for t in train_sets}
    max_iter = {t: args.nepoch * epoch_size[t] for t in epoch_size}
    ...
    for ep in range(args.nepoch):
        ...
        for task in prefetcher:
            for it in range(epoch_size[task]):
                iteration = it + ep * epoch_size[task]
                ...
                adjust_learning_rate(optimizer, args.lr, iteration,
                        args.warm_iter, max_iter[task])
                seggt = None
                ...
                if "seg" in task:
                    ...
                    if iteration >= 0.8*max_iter[task]:
                        new_size = args.size
                    elif args.size == 320:
                ...
                # logging
                if iteration % 100 == 0:
                    logger.info('epoch {} , task {}, iter {}/{}, lr {:.6f}, '
                            'l:loc {:.2f} l:cls {:.2f} '\
                            'l:seg {:.2f} (LOSS {:.3f}) '\
                            'best:{} '\
                        ...
                        load_time,
                        load_time * (max_iter[task] - iteration) / 3600,
                        ))
                    timer.clear()
