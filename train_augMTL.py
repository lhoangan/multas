...
def filter_empty_seg(out:           dict,
                     seggt:         torch.Tensor,
                     ignored_label: int):

    if 'seg' not in out or seggt is None:
        return (None, None)

    filtered = [(o, s) for (o, s) in zip(out['seg'], seggt) if (s != ignored_label).any()]
    if len(filtered) == 0:
        return None, None

    out_, seg_ = zip(*filtered)
    return {'seg': torch.stack(out_)}, torch.stack(seg_)
    ...
    for iteration in range(max_iter):
        ...
        for task in prefetcher:
            ...
            if seggt is not None:
                seggt =  nn.functional.interpolate(seggt.float(),
            ...
            with torch.cuda.amp.autocast():
                ...
                loss += (result['loss_l'] + result['loss_c']) * args.task_weights['det']

                out_, seggt_ = filter_empty_seg(out, seggt, train_sets[task].ignore_label)
                if seggt_ is not None:
                    result = criterion_seg(out_, seggt_)
                    results['loss_s'] += result['loss_seg'] * args.task_weights['seg']
                    loss += result['loss_seg'] * args.task_weights['seg']
