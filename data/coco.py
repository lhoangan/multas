...
class COCODetection(data.Dataset):
    ...
    def __init__(self, image_sets, size, dataset_name='COCO2017', cache=True,
        ...
        cache = False if subset is not None else cache
        for (year, image_set) in image_sets:
            ...
            print (f"Task {task} | Number of classes with BG: {self.num_classes}")
            ...
            if pseudo_seg:
                self.pse.extend([self.pseudo_from_index(coco_name, index) for index in indexes])
            if image_set.find('test') != -1:
                ...
                if image_set.find('val') != -1:
                    ...
                    ids, annotations, pse = [], [], []
                    for j, (i, a) in enumerate(zip(self.ids, self.annotations)):
                        # excluding images with no annotations
                        if a.shape[0] > 0:
                            ids.append(i)
                            annotations.append(a)
                            if pseudo_seg:
                                pse.append(self.pse[j])
                    self.ids = ids
    ...
    def pseudo_from_index(self, name, index):
        """ Construct an image path """
        file_name = (str(index).zfill(12) + '.png')
        image_path = osp.join(self.root,f"annotations/things_{name}_MGGC", file_name)
        assert osp.exists(image_path), 'Path does not exist: {}'.format(image_path)
        return image_path
    ...
    def pull_pseudo_segment(self, index, resize=False):
        """
        For some

        """
        temp = np.array(Image.open(self.pse[index]))

        # NOTE:The GT detection and segmentation have these 11 classes removed
        # 12 (street-sign), 26 (hat), 29 (shoe), 30 (eye-glasses), 45 (plate),
        # 66 (mirror), 68 (window), 69 (desk), 71 (door), 83 (blender), 91 (hair-brush)
        # src: https://github.com/nightrome/cocostuff/blob/master/labels.md
        # but the other classes are not re-numbered. So there are 80 classes
        # with numbering up to 90. Here we have to remap them back to the
        # continuous range of 0-80. Detect is done in _annotation_from_index

        # print ("Segmentation unique: ", np.unique(labelMap))
        # NOTE: ad-hoc way
        # removed_label = [12, 26, 29, 30, 45, 66, 68, 69, 71, 83, 91] # 11 classes
        # for i, lab in enumerate(removed_label):
        #     lab = lab - i
        #     labelMap[labelMap == lab] = 0
        #     mask = (labelMap > lab)
        #     labelMap[mask] = labelMap[mask] - 1
        coco_cat_id_to_class_ind = dict([(self._class_to_coco_cat_id[cls],
                                          self._class_to_ind[cls])
                                         for cls in self._classes[1:]])
        # reshape back to GT size
        labelMap = np.zeros_like(temp)
        for l in np.unique(temp).astype(int):
            if l == 0:
                continue
            labelMap[temp == l] = coco_cat_id_to_class_ind[l]
        if resize:
            labelMap = np.array(Image.fromarray(labelMap).resize(
    ...
    def pull_segment(self, index, resize=False):
        ...
        """
        temp = cocoSegmentationToSegmentationMap(self._COCO,
    ...
    def evaluate_segmentation(self, all_segs, all_thresh=True, per_class=False):
        ...
        class_ind_to_coco_cat_id = dict([(self._class_to_ind[cls],
        ...
        for i, pred in enumerate(all_segs):
            ...
            temp = np.array(Image.fromarray(pred.astype(np.uint8)).resize(
                    (gt.shape[1], gt.shape[0]), resample=Image.NEAREST))
            pred = np.zeros_like(temp)
            for l in np.unique(temp):
                if l == 0:
                    continue
                pred[temp == l] = class_ind_to_coco_cat_id[l]
            preds.append(pred)
...
if __name__ == "__main__":
    ...
    pseudo_seg = True
    dataset = COCODetection([('2017', 'train')], size=320, cache=False, task='det',
                                is_training=False, pseudo_seg=pseudo_seg,
                                subset="Ei2ghth",
                            )
    # detection
    imgs = []
    ...
    for i in range(5):
        ...
        img = img[..., -1::-1].copy()
        det_labels.append(np.unique(data['bboxes'][0].numpy()[:, -1]))
        print("Detection Classes: ", det_labels[-1])
        plt.figure()
        ...
        # showing pseudo seg mask
        if pseudo_seg:
            seg_labels = np.unique(data['mask'][0])
            print("Segmentation Classes: ", seg_labels)
    ...
    for i in range(5):
        ...
        # NOTE:To test label set between detection and segmentation
        # if i < len(det_labels):
        #     if len(det_labels[i]) != len(seg_labels[1:]) or \
        #         (det_labels[i].astype(int) != seg_labels[1:]).any():
        #         plt.figure()
        #         plt.imshow(img)
        #         plt.imshow(data['mask'][0].squeeze(), alpha=.8)
        #         if i < len(imgs):
